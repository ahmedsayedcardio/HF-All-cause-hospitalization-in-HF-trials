...
y <- y[hf_hosp_prop < median(hf_hosp_prop, na.rm = T)]
...
surrogacy_form <- bf(paste0(effect_size,
          " ~
                       0 +
                       outcome +
                       (0 + outcome:hf_hosp_prop + outcome|trial) +", #Nest the 2 outcomes within their trials
...
stes_ranges <- list(
  ...
  seq(-1.85, 0.75, 0.01), #For log-RRs
  seq(-0.28, 0.09, 0.001) #For ARDs
)

#And setup a standard error implied (biggest 5 trials, smallest 5 trials, and the rest in between)
ste_standard_errors_ranges <- list(
  c(
    big_trial_se <- y$logor_se %>%
      sort %>% {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean,
    small_trial_se <- y$logor_se %>% sort(decreasing = TRUE) %>%
      {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean
    ),
  c(
    big_trial_se <- y$logrr_se %>%
      sort %>% {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean,
    small_trial_se <- y$logrr_se %>% sort(decreasing = TRUE) %>%
      {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean
    ),
  c(
    big_trial_se <- y$ard_se %>%
      sort %>% {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean,
    small_trial_se <- y$ard_se %>% sort(decreasing = TRUE) %>%
      {.[1:ceiling(uniqueN(y$trial)/2)]} %>%
      mean
    )
)
...
surrogacy_thresholds <- foreach(
  ...
  .combine = "cbind") %do% {
    ...
    #Surrogacy is defined for non-zero effects and effects > 10% reduciton in odds/risk as well as effects > 1% risk reduction for absolute risk differences
...
reporting_trends_p <- glm(data = xfull, family = binomial,
...
nyha_report_p <- glm(data = xfull, family = binomial,
...
lvef_report_p <- glm(data = xfull, family = binomial,
...
intvn_class_report_p <- glm(data = xfull, family = binomial,
...
hf_type_report_p <- glm(data = xfull, family = binomial,
